...
export class ChunkManager {
  ...
  private buildArgs(generationId: number, originX: number, originZ: number): ChunkQueueArgs {
    const loadDistance = this.renderDistance + 1;
    return {
      generationId,
      originX,
      originZ,
      ...
      loadDistance,
      evictDistance: loadDistance + 2,
      seed: this.seed,
      chunkOrigins: buildGenerationOrder(originX, originZ, loadDistance),
    };
  }
  ...
  setRenderDistance(renderDistance: number, wx: number, wz: number): void {
